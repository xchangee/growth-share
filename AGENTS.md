...
Keep the public project identity standardized as `Open Creative Wall`, the browser title as `开放创意墙｜Open Creative Wall`, and the repository/Pages slug as `open-creative-wall`.

Keep the README online-preview section ordered as 01 profile, 02 avatar letters and 03 main visual. Each preview image must continue linking to `https://xchangee.github.io/open-creative-wall/`, and replacing a screenshot requires updating its exact hash and clearance record in `ASSET-PROVENANCE.md`.
