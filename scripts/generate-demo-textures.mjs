...
// Lucide icons share a 24x24 viewBox, but a few visible geometry bounds are not
// centered on (12, 12). These source-coordinate corrections align each icon's
// rendered bounds with its seal frame without introducing per-cell offsets.
const PATTERN_ICON_CENTER_CORRECTIONS = {
  gem: { x: 0, y: -0.5 },
  leaf: { x: 0.5, y: 0.5 },
  'moon-star': { x: -0.5, y: 0 },
  shapes: { x: 0, y: 0.18 },
...
function createSealMotif(name, inner, centerX, centerY, index, variant) {
  ...
  const iconCorrection = PATTERN_ICON_CENTER_CORRECTIONS[name] ?? { x: 0, y: 0 }
  const iconTranslateX = -12 + iconCorrection.x
  const iconTranslateY = -12 + iconCorrection.y

  return [
    // Keep every generated seal orthogonal to the 24x13 source grid. Rotation
    // jitter makes otherwise centered motifs read as misaligned in narrow crops.
    `  <g transform="translate(${number(centerX)} ${number(centerY)}) scale(${number(unitScale)})"`,
    '     fill="none" stroke="#ffffff" stroke-width="2.05" stroke-linecap="round" stroke-linejoin="round">',
    ...createSealFrame(variant),
    `    <g transform="scale(${number(iconScale)}) translate(${number(iconTranslateX)} ${number(iconTranslateY)})" stroke-width="1.85">`,
    ...inner.split('\n').map((line) => `      ${line.trim()}`),
    '    </g>',
    '  </g>',
  ].join('\n')
...
async function createPatternTexture(iconMap) {
  ...
  for (let index = 0; index < COLUMNS * ROWS; index += 1) {
    ...
    cells.push(createSealMotif(name, inner, centerX, centerY, index, variant))
  }

  return [
    '<!-- Generated by scripts/generate-demo-textures.mjs from Lucide icons (ISC) and project-authored seal frames. -->',
    // CSS isolates one 24x13 sprite cell by stretching the mask image to an
    // explicit 24:13 viewport. Disable the SVG's default aspect-ratio
    // letterboxing so each requested cell remains centered in its crop.
